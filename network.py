...
class ViewFusion_Model(nn.Module):
    def __init__(self, num_views, Z_dims, U_dim):
        super(ViewFusion_Model, self).__init__()
    ...
    def forward(self, Z_list):
        ...
        for v, Z_v in enumerate(Z_list):
            W_v_output = self.fc_layers[v](Z_v)  # Z^v @ W_v, already with activation
...
class AdaM_MVC(nn.Module):
    def __init__(self, view, input_size, class_num,low_feature_dim, high_feature_dim, device):
        super(AdaM_MVC, self).__init__()
        self.encoders = []
        self.decoders = []
        for v in range(view):
        ...
        self.weights = ViewFusion_Model(view, self.view_dims, self.consensus_dim)
        self.Specific_view = nn.Sequential(
            nn.Linear(low_feature_dim, high_feature_dim),
        )
        self.label_contrastive_module = nn.Sequential(
            nn.Linear(low_feature_dim, class_num),#512,10
            nn.Softmax(dim=1)
)
        self.label_contrastive_module_high = nn.Sequential(
            nn.Linear(high_feature_dim, class_num),#512,10
            nn.Softmax(dim=1)
)
    ...
    def AdaM(self, xs):
        ...
        for v in range(self.view):
            qhf = normalize(self.Common_view_F(split_commonz[v]), dim=1)
            qhfs.append(qhf)
            qh = self.label_contrastive_module_high(qhf)
            qhs.append(qh)
        qhs.append(commonz_fused_high_qhs)
        qhfs.append(commonz_fused_high)
        
        return commonz_fused_high, commonz_fused_high_qhs, S, qhs,qhfs,weights_test,split_commonz,split_commonz
